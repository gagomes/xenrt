...
class TCQoSNetwork(xenrt.TestCase):
    ...
    def run(self, arglist=None):
        ...
        self.runSubcase("rate", (100), "LinuxGuest", "rate100")
        self.runSubcase("rate", (1000), "LinuxGuest", "rate1000")
        self.runSubcase("rate", (5000), "LinuxGuest", "rate5000")

    def rate(self, target=100):

        g = self.qosguest
        
        # Remove the guest's VIF and create a new one with a rate limit
        # of 100KBytes/sec
        g.recreateVIFs(rate=target)
        
        # Start the guest and check the rate limit is being applied
        vifs = g.listVIFs()
        if not vifs.has_key("eth0"):
            raise xenrt.XRTError("Could not find guest eth0 in vif-list")
        mac, ip, vbridge, rate = vifs["eth0"]
        if rate != target:
            raise xenrt.XRTFailure("vif-list reports eth0 rate %u, not %u" %
                                   (rate, target))
        g.start()
        measuredlim = self.testRate(g)
        self.tec.comment("%u limited guest rate %u KBytes/sec" %
                         (target, measuredlim))
        ...
        # Start the guest and check the rate limit is not being applied
        vifs = g.listVIFs()
        if not vifs.has_key("eth0"):
            raise xenrt.XRTError("Could not find guest eth0 in vif-list")
        mac, ip, vbridge, rate = vifs["eth0"]
        ...
        g.start()
        measured = self.testRate(g)
        self.tec.comment("Unlimited guest rate %u KBytes/sec" %
                         (measured))
        g.shutdown()
        ...
        wl = target * 2
        if measured < wl:
            self.tec.warning("Unlimited rate %u KBytes/sec is not a lot "
                             "higher than the target limit of %u KBytes/sec" %
                             (measured, target))
        ...
        wl = target / 2
        if measuredlim > wh:
            raise xenrt.XRTFailure("Measured rate %u KBytes/sec is more than "
                                   "20%% higher than the target %u KBytes/sec" %
                                   (measuredlim, target))
        if measuredlim < wl:
            raise xenrt.XRTFailure("Measured rate %u KBytes/sec is more than "
                                   "50%% lower than the target %u KBytes/sec" %
                                   (measuredlim, target))
        
    def testRate(self, guest):
        """Perform a transfer from the VM and measure the rate (KBytes/sec)"""
        data = guest.execcmd("iperf -c %s -t -a 2000M -t %s -f K -i 60" % 
                             (self.peer.getAddress(), self.timeout))
        readings = map(int, re.findall("([0-9]+) KBytes/sec", data))
...
class TCRioQoSNetwork(TCQoSNetwork):

    def __init__(self, tcid="TCRioQoSNetwork"):
        TCQoSNetwork.__init__(self, tcid)
