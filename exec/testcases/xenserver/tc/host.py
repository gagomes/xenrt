...
class TC9994(TC9993):
    """Compare the values of SNMP interfaces against the output of ifconfig"""

    def similarValues(self, configValue, snmpValue):
        if configValue != snmpValue:
            if configValue is None or snmpValue is None:
                return False

            diff = configValue - snmpValue
            # Set divisor to the largest of the two values
            divisor = configValue
            if (diff < 0):
                diff = abs(diff)
                divisor = snmpValue

            # Allow a small difference in the values, since snmp and ifconfig data 
            # are retrieved at different times
            allowedDiff = 0.05         # By default, set it to 5%

            # Adjust for small values. 
            # These may need to be re-adjusted when test is put in production
            if divisor < 130000:
                return diff < 40001
            elif divisor < 500000:
                allowedDiff = 0.3      # 30%
                ...
                allowedDiff = 0.25     # 25%
            elif divisor < 10000000:
                allowedDiff = 0.2      # 20%
            elif divisor < 20000000:
                allowedDiff = 0.15     # 15%  
            elif divisor < 50000000: 
                allowedDiff = 0.1      # 10%

            return float(diff)/divisor <= allowedDiff
        return True

    def checkInterfaceValues(self, interface, ifIndex, snmpData, ifconfigData, diff_values=[]):
        # Compare the values returned by SNMP and ifconfig for the given interface.
        # For each metric whose values differ, an entry is added to the diff_values array.

        # MTU
        snmpMTU = None
        r = re.search("ifMtu\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpMTU = int(r.group(1))

        ifconfigMTU = None
        r = re.search("MTU:([0-9]+)", ifconfigData)
        if r:
            ifconfigMTU = int(r.group(1))

        if snmpMTU != ifconfigMTU:
            diff_values.append("For interface %s: "
                               "SNMP returned ifMtu=%u, ifconfig returned MTU=%u" %
                               (interface, snmpMTU, ifconfigMTU))

        # Physical Address
        snmpPhysAddress = None
        r =  re.search("ifPhysAddress\.%u "
                       "([0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2}:[0-9a-f]{2})" % 
                        ifIndex, snmpData)
        if r:
            snmpPhysAddress = r.group(1)
        ...
        r =  re.search("HWaddr "
                       "([0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2}:[0-9A-F]{2})",
                        ifconfigData)
        if r:
            ifconfigHWaddr = r.group(1).lower()

        if snmpPhysAddress != ifconfigHWaddr:
            diff_values.append("For interface %s: "
                               "SNMP returned ifPhysAddress=%s, ifconfig returned HWaddr=%s" %
                               (interface, snmpPhysAddress, ifconfigHWaddr))

        # Received bytes
        snmpInOctets = None
        r = re.search("ifInOctets\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpInOctets = int(r.group(1))

        ifconfigRXbytes = None
        r = re.search("RX bytes:([0-9]+)", ifconfigData)
        if r:
            ifconfigRXbytes = int(r.group(1))

        if not self.similarValues(ifconfigRXbytes, snmpInOctets):
            diff_values.append("For interface %s: "
                               "SNMP returned ifInOctets=%u, ifconfig returned RX bytes=%u" %
                               (interface, snmpInOctets, ifconfigRXbytes))

        # Received packets
        # SNMP: InUcastPkts + InNUcastPkts
        snmpInPkts = None
        r = re.search("ifInUcastPkts\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpInPkts = int(r.group(1))
        r = re.search("ifInNUcastPkts\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            if snmpInPkts:
                snmpInPkts += int(r.group(1))
            else:
        ...
        ifconfigRXpackets = None
        ifconfigRXerrors = None
        ifconfigRXdropped = None 
        r = re.search("RX packets:([0-9]+) errors:([0-9]+) dropped:([0-9]+)", ifconfigData)
        if r:
            ifconfigRXpackets = int(r.group(1))
            ifconfigRXerrors = int(r.group(2))
            ifconfigRXdropped = int(r.group(3))

        if not self.similarValues(ifconfigRXpackets, snmpInPkts):
            diff_values.append("For interface %s: "
                               "SNMP returned InPkts(ifInUcastPkts+ifInNUcastPkts)=%u, "
                               "ifconfig returned RX packets=%u" %
                               (interface, snmpInPkts, ifconfigRXpackets))

        # Received errors
        snmpInErrors = None
        r = re.search("ifInErrors\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpInErrors = int(r.group(1))

        if not self.similarValues(ifconfigRXerrors, snmpInErrors):
            diff_values.append("For interface %s: "
                               "SNMP returned ifInErrors=%u, ifconfig returned RX errors=%u" %
                               (interface, snmpInErrors, ifconfigRXerrors))

        # Received dropped
        # SNMP: InDiscards + InUnknownProtos
        snmpInDiscards = None
        r = re.search("ifInDiscards\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpInDiscards = int(r.group(1))
        r = re.search("ifInUnknownProtos\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            if snmpInDiscards:
                snmpInDiscards += int(r.group(1))
            else:
        ...
        if not self.similarValues(ifconfigRXdropped, snmpInDiscards):
            diff_values.append("For interface %s: "
                               "SNMP returned ifInDiscards=%u, ifconfig returned RX dropped=%u" %
                               (interface, snmpInDiscards, ifconfigRXdropped))

        # Transmitted bytes
        snmpOutOctets = None
        r = re.search("ifOutOctets\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpOutOctets = int(r.group(1))

        ifconfigTXbytes = None
        r = re.search("TX bytes:([0-9]+)", ifconfigData)
        if r:
            ifconfigTXbytes = int(r.group(1))

        if not self.similarValues(ifconfigTXbytes, snmpOutOctets):
            diff_values.append("For interface %s: "
                               "SNMP returned ifOutOctets=%u, ifconfig returned TX bytes=%u" %
                               (interface, snmpOutOctets, ifconfigTXbytes))

        # Transmitted packets
        # SNMP: OutUcastPkts + OutNUcastPkts
        snmpOutPkts = None
        r = re.search("ifOutUcastPkts\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpOutPkts = int(r.group(1))
        r = re.search("ifOutNUcastPkts\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            if snmpOutPkts:
                snmpOutPkts += int(r.group(1))
            else:
        ...
        ifconfigTXpackets = None
        ifconfigTXerrors = None
        ifconfigTXdropped = None
        r = re.search("TX packets:([0-9]+) errors:([0-9]+) dropped:([0-9]+)", ifconfigData)
        if r:
            ifconfigTXpackets = int(r.group(1))
            ifconfigTXerrors = int(r.group(2))
            ifconfigTXdropped = int(r.group(3))

        if not self.similarValues(ifconfigTXpackets, snmpOutPkts):
            diff_values.append("For interface %s: "
                               "SNMP returned OutPkts(ifOutUcastPkts+ifOutNUcastPkts)=%u, "
                               "ifconfig returned TX packets=%u" %
                                (interface, snmpOutPkts, ifconfigTXpackets))

        # Transmitted errors
        snmpOutErrors = None
        r = re.search("ifOutErrors\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpOutErrors = int(r.group(1))

        if not self.similarValues(ifconfigTXerrors, snmpOutErrors):
            diff_values.append("For interface %s: "
                               "SNMP returned ifOutErrors=%u, ifconfig returned TX errors=%u" %
                               (interface, snmpOutErrors, ifconfigTXerrors))

        # Transmitted dropped
        snmpOutDiscards = None
        r = re.search("ifOutDiscards\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpOutDiscards = int(r.group(1))

        if not self.similarValues(ifconfigTXdropped, snmpOutDiscards):
            diff_values.append("For interface %s: "
                               "SNMP returned ifOutDiscards=%u, ifconfig returned TX dropped=%u" %
                               (interface, snmpOutDiscards, ifconfigTXdropped))

        # Length of the transmit queue
        snmpOutQLen = None
        r = re.search("ifOutQLen\.%u ([0-9]+)" % ifIndex, snmpData)
        if r:
            snmpOutQLen = int(r.group(1))

        ifconfigTXqueuelen = None
        r = re.search("txqueuelen:([0-9]+)", ifconfigData)
        if r:
            ifconfigTXqueuelen = int(r.group(1))

        # From CA-33420: SNMP doesn't always update this value and returns 0
        if snmpOutQLen != ifconfigTXqueuelen and snmpOutQLen != 0:
            diff_values.append("For interface %s: "
                               "SNMP returned ifOutQLen=%u, ifconfig returned txqueuelen=%u" %
                               (interface, snmpOutQLen, ifconfigTXqueuelen))
    ...
    def run(self, arglist=None):
        ...
        # Retrieve the interface indices. The data will be returned in this form: 
        # IF-MIB::ifIndex.5 = INTEGER: 5
        # IF-MIB::ifIndex.503 = INTEGER: 503
        data = xenrt.command(\
            "snmpwalk -v 1 -c %s %s interfaces | grep ifIndex" %
            (self.community, self.host.getIP())).strip()
        if not data:
            raise xenrt.XRTFailure("Could not get snmp interface indices")

        ifIndexData = str(data).splitlines()

        # For each interface, fetch and compare the data from SNMP and ifconfig
        diff_values = []  # Stores values which differ between SNMP and ifconfig
        for ifIndexLine in ifIndexData: 
            # Find the interface index
            ifIndex = None
            r = re.search("ifIndex\.([0-9]+)", ifIndexLine)
            if r:
                ifIndex = int(r.group(1))
            if ifIndex is None:
                raise xenrt.XRTFailure("snmp interface index missing",
                                       ifIndexLine)
            
            # Get the name of the interface at ifIndex 
            interface = xenrt.command(\
                "snmpget -v1 -c %s -Ovq %s IF-MIB::ifDescr.%u" %
                (self.community, self.host.getIP(), ifIndex)).strip()
            if not interface:
                raise xenrt.XRTFailure("Could not snmpget interface name",
                                       "ifIndex: %u" % (ifIndex))

            # Get all SNMP data for the interface at ifIndex
            snmpData = xenrt.command(\
                "snmpwalk -O0sq -c %s -v 1 %s interfaces | grep '\.%u '" %
                (self.community, self.host.getIP(), ifIndex)).strip()
            if not snmpData:
                raise xenrt.XRTFailure("Could not get snmp data for interface",
                                       interface)

            # Get ifconfig data for the same interface
            ifconfigData = self.host.execdom0("/sbin/ifconfig %s" % interface)
            if not ifconfigData:
                raise xenrt.XRTFailure("Could not get ifconfig data for interface",
                                       interface)
       
            self.checkInterfaceValues(interface, ifIndex, snmpData, 
                                      ifconfigData, diff_values)

        if diff_values:
            xenrt.TEC().logverbose("The difference between SNMP and ifconfig "
                                   "values is out of limits:\n%s" % 
                                   ("\n".join(diff_values)))
            raise xenrt.XRTFailure("The difference between SNMP and ifconfig "
                                   "values is out of limits", 
                                   "\n".join(diff_values))
