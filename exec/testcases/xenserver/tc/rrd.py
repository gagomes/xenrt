...
# OEM specific TCs
class TC8317(_RRDBase):
    """Verify that RRDs are synced to flash on clean shutdown/reboot, and
       copied back to tmpfs on boot"""
    NEED_SLAVE = False

    def prepare(self, arglist=None):
        if not xenrt.TEC().lookup("OPTION_EMBEDDED", False, boolean=True):
            raise xenrt.XRTError("Cannot verify Flash sync on a non Flash SKU")
        _RRDBase.prepare(self, arglist=arglist)

    def run(self, arglist=None):
        xenrt.TEC().logverbose("Leaving VM for 2 minutes to ensure RRD is populated")
        time.sleep(120)
        xenrt.TEC().logverbose(" ... done")
        ...
        # Check it's in /var/xapi/blobs (and make sure it's a tmfps)
        ts = self.rrdTimestamp(self.guest, self.master)
        if not ts:
            raise xenrt.XRTError("Guest RRD not found in /var/xapi/blobs")
        if not "tmpfs" in self.master.execdom0("df -T /var/xapi/blobs"):
            raise xenrt.XRTFailure("/var/xapi/blobs is not a tmpfs on OEM "
                                   "Flash SKU")
        # Make sure it's not in /var/xapi/blobs.persistent (and make sure this is
        # on flash)
        if self.rrdTimestamp(self.guest, self.master, persistent=True):
            raise xenrt.XRTError("Guest RRD already present in "
                                 "/var/xapi/blobs.persistent")
        if self.master.execdom0("ls /var/xapi/blobs.persistent",
                                retval="code") > 0:
            raise xenrt.XRTFailure("/var/xapi/blobs.persistent does not exist "
                                   "on OEM Flash SKU")
        if "tmpfs" in self.master.execdom0("df -T /var/xapi/blobs.persistent"):
            raise xenrt.XRTFailure("/var/xapi/blobs.persistent is a tmpfs!")

        # Perform a clean shutdown
        self.master.shutdown()
        # Power cycle the host to make it turn back on
        self.master.machine.powerctl.cycle()
        # Wait for it to boot
        self.master.waitForSSH(600, desc="Host boot after clean shutdown")

        # Check the RRD is on the flash
        if not self.rrdTimestamp(self.guest, self.master, persistent=True):
            raise xenrt.XRTFailure("Guest RRD was not synced to flash on clean "
                                   "shutdown")
        # Check it was copied back
        newTS = self.rrdTimestamp(self.guest, self.master)
        if not newTS:
            raise xenrt.XRTFailure("Guest RRD was not synced from flash to "
                                   "tmpfs on startup")
        # Check it has the right timestamp
        if newTS != ts:
            raise xenrt.XRTError("Timestamps on RRDs don't match",
                                 "Before %s, after %s" % (ts, newTS))

        # Remove the RRD from flash
        self.master.execdom0("rm -f /var/xapi/blobs.persistent/rrds/%s.gz" %
                             (self.guest.getUUID()))
...
class TC8316(_RRDBase):
    """Verify that RRDs are periodically synced to flash on flash SKUs"""
    FIST_POINTS = ["fist_reduce_rrd_backup_interval",
                   "fist_reduce_blob_sync_interval"] # Sync every 5m, starting
                                                     # after 10m.
    VM_ON_SLAVE = True
    ...
    def run(self, arglist=None):
        # Leave the VM running for more than 10m
        xenrt.TEC().logverbose("Leaving VM idle for 11 minutes")
        time.sleep(660)
        xenrt.TEC().logverbose(" ... done")

        # Now check we have the RRD on the master in tmpfs
        ts = self.rrdTimestamp(self.guest, self.master)
        if not ts:
            raise xenrt.XRTError("RRD not backed up to master")
        # Check it has gone to flash
        flashTS = self.rrdTimestamp(self.guest, self.master, persistent=True)
        if not flashTS:
            raise xenrt.XRTFailure("RRD not synced to flash on master when "
                                   "expected")
        # We expect it to have been updated on tmpfs, since that happens every
        # 5 mins, starting after 6 mins, so 11 mins in we'll have had a sync
        # at 6 mins, and another at 11 mins. The flash sync will have happend
        # after 10mins, when it will have been the first version, so we expect
        # an ~ 5min difference between the timestamps
        difference = ts - flashTS
        change = abs(300 - difference)
        if change > 10:
            raise xenrt.XRTFailure("Timestamps of RRD on flash and tmpfs aren't"
                                   " as expected on master")

        # Now check the slave (it should have been synced there as well)
        slaveTS = self.rrdTimestamp(self.guest, self.slave)
        if not slaveTS:
            raise xenrt.XRTError("RRD not backed up to slave")
        if slaveTS != flashTS:
            raise xenrt.XRTFailure("Timestamps of RRD on slave differ from "
                                   "master")
        # Check it's gone to flash
        slaveFlashTS = self.rrdTimestamp(self.guest, self.slave, persistent=True)
        if not slaveFlashTS:
            raise xenrt.XRTFailure("RRD not synced to flash on slave when "
                                   "expected")
        if slaveFlashTS != slaveTS:
            raise xenrt.XRTFailure("Timestamps of RRD on flash and tmpfs differ"
                                   "on slave")

# OEM Flash Upgrade Tests

class _RRDFlashUpgBase(_RRDBase):
    """Base class for RRD Flash Upgrade Tests"""
    CLAIMED_DISK = False
    EXISTING_RRDS = False
    ...
    def prepare(self, arglist=None):
        ...
        # First check we are indeed running on Flash...
        if not self.host.embedded:
            xenrt.TEC().skip("Cannot run RRD Flash upgrade tests on non Flash "
                             "SKU")
            return

        # Are we expecting a claimed disk - if so make sure it's there
        if self.CLAIMED_DISK:
            # Make sure it exists
            if self.host.execdom0("mount | grep XenConfig", retval="code") > 0:
                raise xenrt.XRTError("No claimed disk found")
        else:
            # Free it up and reboot
            disks = self.host.getLocalDisks()
            for disk in disks:
                self.host.execdom0("/opt/xensource/libexec/delete-partitions -d"
                                   " %s || true" % (disk))
            self.host.cliReboot()
            # Make sure it doesn't exist now
            if self.host.execdom0("mount | grep XenConfig", retval="code") == 0:
                raise xenrt.XRTError("Claimed disk found after running "
                                     "delete-partitions")
        # Do we need any existing RRDs - if so create one etc...
        # XXX: Does the host RRD always exist here?
        if self.EXISTING_RRDS:
            self.guest = self.host.createGenericLinuxGuest()
            # Wait 10 mins to let the RRD build up
            time.sleep(600)
            self.guest.shutdown()
            # Check the RRD exists
            if not self.rrdTimestamp(self.guest, self.host, gzipped=False):
                raise xenrt.XRTError("RRD not created when expected")            
    def run(self, arglist=None):
        # Do the upgrade
        self.host.upgrade()

        # Verify directories exist in the right places of the right type
        # /var/xapi/blobs should be a ramdisk
        if not "tmpfs" in self.host.execdom0("df -T /var/xapi/blobs || true"):
            raise xenrt.XRTFailure("/var/xapi/blobs doesn't exist or is not a "
                                   "tmpfs after upgrade to update1")
        
        # /var/xapi/blobs.persistent should be on flash (NOT claimed disk)
        if self.host.execdom0("ls /var/xapi/blobs.persistent", 
                              retval="code") > 0:
            raise xenrt.XRTFailure("/var/xapi/blobs.persistent doesn't exist")
        dfPers = self.host.execdom0("df -TP /var/xapi/blobs.persistent")
        if "tmpfs" in dfPers:
            raise xenrt.XRTFailure("/var/xapi/blobs.persistent is a tmpfs")
        persFS = dfPers.splitlines()[1].split()[0]
        dfEtc = self.host.execdom0("df -TP /etc")
        etcFS = dfEtc.splitlines()[1].split()[0]
        if persFS.strip() != etcFS.strip():
            raise xenrt.XRTFailure("/var/xapi/blobs.persistent does not appear "
                                   "to be on flash")

        if self.EXISTING_RRDS:
            # The existing RRDs should exist in both /var/xapi/blobs.persistent
            # and /var/xapi/blobs
            if not self.rrdTimestamp(self.guest, self.host):
                raise xenrt.XRTFailure("RRD from before upgrade not found in "
                                       "/var/xapi/blobs")
            if not self.rrdTimestamp(self.guest, self.host, persistent=True):
                raise xenrt.XRTFailure("RRD from before upgrade not found in "
                                       "/var/xapi/blobs.persistent")

#  Without claimed disk
class TC8319(_RRDFlashUpgBase):
    """Upgrade to Orlando Update 1 from Orlando GA on OEM Flash without claimed
       disk"""
    pass

#  With claimed disk
#   Without existing RRDs
class TC8320(_RRDFlashUpgBase):
    """Upgrade to Orlando Update 1 from Orlando GA on OEM Flash with claimed
       disk, and no existing RRDs"""
    CLAIMED_DISK = True

#   With existing RRDs
class TC8321(_RRDFlashUpgBase):
    """Upgrade to Orlando Update 1 from Orlando GA on OEM Flash with claimed
       disk, and existing RRDs"""
    CLAIMED_DISK = True
    EXISTING_RRDS = True
