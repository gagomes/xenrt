...
class SourceGenerator(NodeVisitor):
    ...
    def visit_Exec(self,node):
        self.newline(node)
        self.write('exec(')
        if type(node.body) == Name:
            self.write(node.body.id)
        elif type(node.body) == Str:
            self.write("'%s'"%(node.body.s))
        self.write(')')
        if node.globals != None and node.locals != None:
            self.write(' in ')
            self.write(node.globals.func.id)
            self.write('(),')
            self.write(node.locals.func.id)
            self.write('()')
    ...
    def visit_Cont(self, node):
        self.newline(node)
        self.write('xenrtdebugger_breakpoint ')
        self.visit(node.test)
        #self.write(' in ')

    def visit_Auto(self,node):
        self.newline(node)
        self.write('xenrtdebugger_errormarker')
    ...
    def visit_Br(self,node):
        self.newline(node)
        self.write('xenrtdebugger_iteratebreakpoint ')
        self.visit(node.target)
        self.write(' in ')
        self.visit(node.iter)
        self.write(':')
    ...
    def visit_Until(self, node):
        self.newline(node)
        self.write('xenrtdebugger_silentbreakpoint ')
        #self.visit(node.target)
        #self.write(' in ')
#        self.visit(node.test)
 #       self.write(':')
