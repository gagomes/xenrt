...
class SXProcess(object):
    ...
    def deploy(self, providerId, host, template, profileName=None):
        ...
        result = self.apiHandler.execute(category="deploymentprofile", method="POST", params={"processIdStr": self.processId, "processVersionStr": self.processVersion, "deploymentProfileName": profileName, "profileDetailsStr": json.dumps(deploymentProfile['profileDetails'], ensure_ascii=False).encode('utf8')})
