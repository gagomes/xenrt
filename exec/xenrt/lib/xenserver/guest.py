...
""" % (string.join(u, "\n")) 
    ...
    def _cloneCopyVM(self, operation, name, timer, sruuid, uuid=None, noIP=True):
        ...
        g.vifsN = ((g.managenetwork or g.managebridge)
                and g.getVIFs(network=g.managenetwork, \
                              bridge=g.managebridge).keys()
                or g.vifs)

        ips = filter(None, map(lambda (nic, vbridge, mac, ip):ip, g.vifsN))
        if ips and not noIP:
            g.mainip = ips[0]
        g.vifsN.sort()
        if g.use_ipv6 and g.vifsN:
            g.mainip = g.getIPv6AutoConfAddress(device=g.vifsN[0][0])
        elif g.mainip: 
