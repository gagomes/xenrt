...
class DundeeHost(CreedenceHost):
    ...
    def install(self,
        ...
        self.password = self.lookup("ROOT_PASSWORD")
        ...
        self.setupExtraInstallPackages(packdir, extracds, suppackcds)

        # Create an NFS directory for the installer to signal completion
        self.signaldir = xenrt.NFSDirectory()
        
        ansfile = self.createInstallAnswerfile(upgrade,
                                               primarydisk,
                                               guestdisks,
                                               interfaces,
                                               source,
        ...
        if async:
            ...
            return handle
        handle = (self.signaldir, packdir, pxe)
        if xenrt.TEC().lookup("OPTION_BASH_SHELL", False, boolean=True):
            xenrt.TEC().tc.pause("Pausing due to bash-shell option")
...
""" % (dom0cpus, dom0cpus)
    ...
    def postInstallV6(self, upgrade):
        v6hack = ""
        mockd = xenrt.TEC().getFile(self.V6MOCKD_LOCATION)
        if upgrade and not mockd:
            # Set up a temporary WWW directory to hold the v6testd
...
""" % (disk, disk, srtype, srtype)
    ...
    def postInstallOverlay(self, overlay):
        if overlay:
            overlaynfsurl = overlay.getMountURL("")
            ret = """
# Apply the overlay
...
""" % self.signaldir.getMountURL("")
    ...
    def getCD(self):
        ...
        if not self.cd:
            ...
            imagePath = xenrt.TEC().lookup("CD_PATH_%s" % self.productVersion.upper(), 
                                           xenrt.TEC().lookup('CD_PATH', 'xe-phase-1'))
            self.cd = xenrt.TEC().getFile(os.path.join(imagePath, imageName), imageName)
        if not self.cd:
            raise xenrt.XRTError("No CD image supplied.")
        xenrt.checkFileExists(self.cd)

    def setupExtraInstallPackages(self, packdir, extracds, suppackcds): 
        workdir = xenrt.TEC().getWorkdir()
        # If we have any extra CDs, copy the extra packages as well
        if extracds:
            ecds = extracds
        ...
        if suppackcds or supptarballs or suppdirs:
            repofile = "%s/XS-REPOSITORY-LIST" % (workdir)
            repo = file(repofile, "w")
            f = file("%s/XS-REPOSITORY-LIST" % (packdir), "r")
            repo.write(f.read())
            f.close()
            if supptarballs:
                for supptar in supptarballs.split(","):
    ...
    def createInstallAnswerfile(self,
        ...
        if self.lookup("LOCAL_SR_POST_INSTALL", False, boolean=True): 
            ...
            if installSRType:
                ...
                self.firstBootSRInfo = (guestdisks[0], self.lookup("INSTALL_SR_TYPE", defaultSRType))
            guestdisks = []
...
""" % (primarydiskconfig, guestdiskconfig)
        if upgrade:
            ...
            rpassword = "<root-password>%s</root-password>" % (self.password)
...
""" % (installationExtras,
    ...
    def createPostInstallFiles(self,
        ...
        postInstall.append(self.postInstallV6(upgrade))
        ...
        postInstall.append(self.postInstallOverlay(overlay))
