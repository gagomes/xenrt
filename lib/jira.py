...
class JiraIssue(JiraObject):
    ...
    def setAssignee(self,assignee):
        ...
        self.update([{'id': 'assignee','values': assignee}])    
    ...
    def linkIssue(self,linkTo,linkType):
        ...
        url = "%s/rest/api/2/issueLinkType?os_username=%s&os_password=%s" % (self.Jira.url, urllib.quote_plus(self.Jira.username), urllib.quote_plus(self.Jira.password))

        try:
            linkTypes = eval(urllib2.urlopen(url).read())
        except:
            ...
            links = {}
            for i in linkTypes["issueLinkTypes"]:
                links[i["inward"]] = (i["name"], True)
                links[i["outward"]] = (i["name"], False)
            (linkTypeName, inward) = links[linkType]
            if inward:
                postData = "{\"type\":{\"name\":\"%s\"}, \"inwardIssue\":{\"key\":\"%s\"}, \"outwardIssue\":{\"key\":\"%s\"} }" % (linkTypeName, linkTo, self.key)
            else:
                postData = "{\"type\":{\"name\":\"%s\"}, \"inwardIssue\":{\"key\":\"%s\"}, \"outwardIssue\":{\"key\":\"%s\"} }" % (linkTypeName, self.key, linkTo)
            postURL = "%s/rest/api/2/issueLink?os_username=%s&os_password=%s" % (self.Jira.url, urllib.quote_plus(self.Jira.username), urllib.quote_plus(self.Jira.password))
            try:
                urllib2.urlopen(urllib2.Request(postURL, postData,{'Content-Type': 'application/json'}))
            except Exception, e:
                if not re.search("HTTP Error 201: Created", str(e)):
                    raise
    ...
    def getLinks(self):
        ...
        postURL = ("%s/si/jira.issueviews:issue-xml/%s/?os_username=%s&"
                  "os_password=%s" % (self.Jira.url,self.key,urllib.quote_plus(self.Jira.username),
                                      urllib.quote_plus(self.Jira.password)))
    ...
    def _deleteLink(self, id, destId, linkType):
        ...
        postURL = ("%s/secure/DeleteLink.jspa?id=%s&destId=%s&linkType=%s&"
                   ...
                   (self.Jira.url,id,destId,linkType,urllib.quote_plus(self.Jira.username),
                    urllib.quote_plus(self.Jira.password)))
    ...
    def copyAttachmentsTo(self, destination):
        ...
        auth = "os_username=%s&os_password=%s" % (urllib.quote_plus(self.Jira.username), 
                                                  urllib.quote_plus(self.Jira.password))
