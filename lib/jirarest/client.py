...
import json
from jirarest.exceptions import raise_on_error
from jirarest.resources import Resource, Issue, Comment, Project, Attachment, Component, Dashboard, Filter, Votes, Watchers, Worklog, IssueLink, IssueLinkType, IssueType, Priority, Version, Role, Resolution, SecurityLevel, Status, User, CustomFieldOption, RemoteLink
from jirarest.resources import Board, Sprint
...
class GreenHopper(JIRA):
    ...
    def sprints(self, id, includeHistoricSprints=True, includeFutureSprints=True):
        '''
        Return the Sprints that appear with the given board id

        Example: rest/greenhopper/1.0/sprintquery/2?includeHistoricSprints=true&includeFutureSprints=true
        '''
        r_json = self._gh_get_json('sprintquery/%s?includeHistoricSprints=%s&includeFutureSprints=%s' %
                                     (id,
                                     "true" if includeHistoricSprints else "false",
                                     "true" if includeFutureSprints else "false"))
