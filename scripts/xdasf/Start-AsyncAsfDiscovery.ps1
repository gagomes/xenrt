...
# Encrypt an xml blob for trusted porbe attempt
Function EncryptedXml
{
	if($certFilePath -ne [string]::Empty)
	{
		$probeextensionxml = "<probe trusted='true'/>" # xml blob used for sending encrypted trusted discovery

		$CertFilePath = Resolve-Path $CertFilePath
	
		Write-Verbose "EncryptedXml: Create an encrypted xml blob so the server can validate that a trusted certificate is on this machine"
		$xmlDoc = New-Object -TypeName System.Xml.XmlDocument
		$xmlDoc.LoadXml($probeextensionxml)

		$cert = New-Object -TypeName System.Security.Cryptography.X509Certificates.X509Certificate2 -ArgumentList $CertFilePath
		$elementToEncrypt = $xmlDoc.DocumentElement

		$encryptedXml = New-Object -TypeName Security.Cryptography.Xml.EncryptedXml 

		$edElement = $encryptedXml.Encrypt($elementToEncrypt, $cert)

		[System.Security.Cryptography.Xml.EncryptedXml]::ReplaceElement($elementToEncrypt, $edElement, $false)
		
		Write-Debug "EncryptedXml: $($xmlDoc.OuterXml)"
...
# if a certificate is specified this creates an encrypted xml block
# that will be included in the discovery probe soap packet
# $probeextensionxml = EncryptedXml
